...
This script will loop through all the block devices on a node and return
all the bcache devices along with whether they are caching or backing.
"""
...
def enumerate_disks():
        disks = check_output("lsblk -l | tail -n +2 | awk '{print $1}'",
                             shell=True).split()
        Caches = []
        bcaches = {}
        for i in disks:
...
def get_bcaches(cache_disk, backing_disks):
    for i in cache_disk:
        ...
        disks = []
        for b in b_caches:
                backing_disks[b] = backing_disks[b] + \
                    (get_state(backing_disks[b])).rstrip()
