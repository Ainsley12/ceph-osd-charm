...
class CephOsdBasicDeployment(OpenStackAmuletDeployment):
    ...
    def test_913_show_bcache_devs_action(self):
        """Execute the show-bcache-actions action"""
        u.log.debug("Testing show-bcache-devs action")

        sentry_unit = self.ceph0_sentry
        action_id = u.run_action(sentry_unit, 'show-bcache-devs')
        # Check whether the action ran without errors
        assert u.wait_on_action(action_id) != 'failed'
